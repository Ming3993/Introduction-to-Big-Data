...
1. The **\[Debug section]** is intended for inspecting output during development. Do **not** run these cells if your goal is simply to publish the z-score results to MongoDB.

2. In the last cell of the notebook, the code will throw an error if required Kafka topics such as `btc-price` and `btc-price-moving` do not yet exist. To check whether these topics are available, open the Kafka UI mentioned earlier and go to the **Topics** section (see illustration below).
